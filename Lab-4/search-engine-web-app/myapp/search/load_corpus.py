...
def load_corpus(path) -> [Document]:
    ...
    :return:
    """
    corpus = _load_corpus_as_dataframe(path)
...
def _load_corpus_as_dataframe(path):
    ...
    corpus = tweets_df.rename(
        columns={"id": "Id", "text": "Tweet", "username": "Username", "date": "Date",
                 "like": "Likes",
                 "rt": "Retweets", "lang": "Language"
                 ,"URL": "Url",
                 "hashtag": "Hashtags"})
...
def _load_tweets_as_dataframe(json_data):
    ...
    for line in json_data:
        tweets.append({
            'id' : int(line.get('id')),
            'text': line.get('full_text'),
            'username' : line.get('user').get('screen_name'),
            'date' : line.get('created_at'),
            'hashtag' : list(map(lambda hashtag:  hashtag.get('text'),  line.get('entities').get('hashtags'))),
            'like' : line.get('favorite_count'),
            'rt' : line.get('retweet_count'),
            'URL' : 'https://twitter.com/' + line.get('user').get('screen_name') + "/status/" + str(line.get('id')),
            'lang' : line.get('lang')
